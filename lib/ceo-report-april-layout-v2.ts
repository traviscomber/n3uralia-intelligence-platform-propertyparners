...
function salesChart(monthly: Array<{ short: string; actual: number; target: number }>) {
  ...
  return monthly.map((m) => {
    ...
    return `<tr><td width="44" style="padding:9px 10px 9px 0;font:700 11px Calibri,Arial,sans-serif;color:${BRAND.foreground}">${m.short}</td><td style="padding:9px 0"><table width="100%" role="presentation" cellpadding="0" cellspacing="0" style="border-collapse:collapse"><tr><td width="76" style="font:10px Calibri,Arial,sans-serif;color:${BRAND.muted}">Cierres ${fmt(m.actual)}</td><td><div style="height:10px;background:${BRAND.surfaceMuted}"><div style="height:10px;width:${actualW}%;background:${BRAND.chartBlue}"></div></div></td></tr><tr><td width="76" style="padding-top:5px;font:10px Calibri,Arial,sans-serif;color:${BRAND.muted}">Meta ${fmt(m.target,1)}</td><td style="padding-top:5px"><div style="height:5px;background:${BRAND.surfaceMuted}"><div style="height:5px;width:${targetW}%;background:${BRAND.primary}"></div></div></td></tr></table></td></tr>`
  }).join('')
}
...
function complianceChart(monthly: Array<{ short: string; compliance: number }>) {
  ...
  const dots = points.map((p) => `<circle cx="${p.x}" cy="${p.y}" r="4" fill="${BRAND.chartBlue}"/><text x="${p.x}" y="${p.y-10}" text-anchor="middle" font-size="10" font-weight="700" fill="${BRAND.foreground}">${fmt(p.compliance)}%</text><text x="${p.x}" y="${H-10}" text-anchor="middle" font-size="10" fill="${BRAND.muted}">${p.short}</text>`).join('')
  return `<svg viewBox="0 0 ${W} ${H}" width="100%" role="img" aria-label="Evolución del cumplimiento mensual" xmlns="http://www.w3.org/2000/svg"><rect width="${W}" height="${H}" fill="${BRAND.surface}"/>${grid}<line x1="${left}" y1="${top + chartH - 100/110*chartH}" x2="${W-20}" y2="${top + chartH - 100/110*chartH}" stroke="${BRAND.primary}" stroke-dasharray="4 4"/><polyline points="${polyline}" fill="none" stroke="${BRAND.chartBlue}" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/>${dots}</svg>`
}
...
export async function generateCeoReportAprilLayoutV2(monthName: string, period: string) {
  ...
  <tr><td style="padding:38px 44px;border-bottom:1px solid ${BRAND.border}">${sectionTitle('02','VENTA DEL MES','Cierres versus meta','Comparación visual mensual siguiendo la lógica de las presentaciones canónicas.')}<table width="100%" role="presentation" cellpadding="0" cellspacing="0" style="background:${BRAND.surface};border:1px solid ${BRAND.border};padding:18px"><tr><td style="padding:16px 20px"><table width="100%" role="presentation" cellpadding="0" cellspacing="0">${salesChart(monthly)}</table><div style="font:10px Calibri,Arial,sans-serif;color:${BRAND.muted};margin-top:12px">Azul: cierres reales · Rojo: meta mensual</div></td></tr></table></td></tr>
