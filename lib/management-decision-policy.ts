...
export const MANAGEMENT_DECISION_POLICY = {
  ...
  rules: [
    ...
    { id: 'lead-backlog-action', metric: 'stale_90_leads_ratio', label: 'Backlog +90 días accionable', operator: '>=', threshold: 20, severity: 'medium', origin: 'n3uralia_provisional', status: 'active_provisional', rationale: 'Activa una acción ejecutiva antes de alcanzar el umbral crítico; no equivale a un KPI contractual.' },
    ...
    { id: 'visits-action', metric: 'realized_visits_rate', label: 'Ejecución de visitas accionable', operator: '<', threshold: 70, severity: 'medium', origin: 'n3uralia_provisional', status: 'active_provisional', rationale: 'Activa una acción ejecutiva de seguimiento antes del nivel crítico; no redefine la metodología canónica de visitas.' },
    ...
    { id: 'unclassified-critical', metric: 'unclassified_leads_ratio', label: 'Leads sin clasificar críticos', operator: '>=', threshold: 40, severity: 'high', origin: 'n3uralia_provisional', status: 'active_provisional', rationale: 'Eleva prioridad cuando una proporción muy alta del universo activo carece de clasificación.' },
...
export function getDecisionThreshold(id: string) {
  const rule = getDecisionRule(id)
  if (!rule) throw new Error(`Missing management decision rule: ${id}`)
