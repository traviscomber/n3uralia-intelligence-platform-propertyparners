...
export async function buildManagementReportPdf(report: ManagementReportRecord) {
  ...
  const period = text(snapshot.period, `${report.period_start} – ${report.period_end}`)
  const provenance = text(snapshot.dataProvenance, 'Snapshot persistido del sistema de control de gestión.')
  ...
  drawLines(provenance, { size: 9, color: muted, gap: 13 })
  ...
  for (const rawEntity of entities.slice(0, 100)) {
    ...
    ensureSpace(72)
    separator(8, 12)
    const entityName = text(entity.name, 'Entidad sin nombre')
    const entityType = text(entity.entity_type ?? entity.entityType, 'entidad')
    drawLines(entityName, { font: bold, size: 12, gap: 17 })
    drawLines(`Tipo: ${entityType}`, { size: 8, color: muted, gap: 13 })

    const metrics = array(entity.metrics)
    if (!metrics.length) {
      drawLines('Sin métricas persistidas para este período.', { size: 9, color: muted, indent: 8, gap: 13 })
      continue
    }
  ...
  const filename = `${filenamePart(reportTypeLabel(report.report_type))}-${report.period_start}-${report.period_end}.pdf`
