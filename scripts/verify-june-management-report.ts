...
async function main() {
  const reportId = '758ed703-845e-4fe4-a6fb-54d1a6b62820'
  ...
  if (error || !data) throw new Error(`May report lookup failed: ${error?.message ?? 'not found'}`)
  if (data.period_start !== '2026-05-01' || data.period_end !== '2026-05-31') throw new Error('Unexpected May period')
  if (data.snapshot?.schemaVersion !== 'canonical-monthly-report-v1') throw new Error('Unexpected report schema')
  if (data.snapshot?.completeness?.operationalReportReady !== true) throw new Error('May report is not operationally ready')
  if (data.snapshot?.completeness?.fullManagementScoreReady !== false) throw new Error('May report must keep incomplete management score blocked')
