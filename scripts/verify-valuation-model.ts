...
assert.match(marketExport, /getMarketHouseIntelligence/, 'Market export must use the canonical house intelligence surface.')
assert.doesNotMatch(marketExport, /market_current_listings|market_transactions/, 'Market export must not bypass the aggregate intelligence boundary.')
assert.match(marketPage, /dataset=summary&format=xlsx/, 'Market dashboard must expose the house-only XLSX summary.')
...
assert.match(marketPrint, /Ventas registradas por barrio/, 'Market report must expose the canonical house-sale neighborhood table.')
