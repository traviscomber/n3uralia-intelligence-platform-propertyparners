...
const dependencies = JSON.parse(fs.readFileSync('config/client-dependencies-status.json', 'utf8'))
...
if (!Number.isInteger(snapshot.clientQuestionsOpen) || snapshot.clientQuestionsOpen < 0) throw new Error('Invalid clientQuestionsOpen')
...
const openDependencies = (dependencies.dependencies ?? []).filter((item) => item.status === 'pending')
const openQuestions = openDependencies.reduce((count, item) => count + (item.questions?.length ?? 0), 0)

if (snapshot.clientDependenciesOpen !== openDependencies.length) {
  throw new Error(`Snapshot clientDependenciesOpen=${snapshot.clientDependenciesOpen} does not match manifest=${openDependencies.length}`)
}
if (snapshot.clientQuestionsOpen !== openQuestions) {
  throw new Error(`Snapshot clientQuestionsOpen=${snapshot.clientQuestionsOpen} does not match manifest=${openQuestions}`)
}
...
if (snapshot.contractStatus === 'closed') {
  ...
  if (snapshot.clientDependenciesOpen > 0 || snapshot.clientQuestionsOpen > 0) throw new Error('Closed contract cannot contain open client dependencies or questions')
...
console.log(`Contract closeout snapshot verified: ${snapshot.contractStatus}; dependencies=${openDependencies.length}; questions=${openQuestions}`)
