...
for (const file of scanRoots.flatMap((directory) => walk(path.join(root, directory)))) {
  ...
  if (/^app\/api\//.test(rel) && /(\.from\(|\.rpc\()/i.test(text)) {
    const hasAuthSignal = /(getUser|getSession|requireAuth|requireRoleAccess|requireExecutiveAccess|requireCapability|requireAnyCapability|requireUserScope|authorize|accessErrorResponse|tenantId|tenant_id|organizationId|organization_id|officeId|office_id|companyId|company_id|CRON_SECRET|authorization)/i.test(text)
    if (!hasAuthSignal) {
