...
async function main() {
  ...
  assert.match(refresh, /MAX_PAGES_PER_DATASET = [2-9]/, 'Portal refresh must discover beyond the first results page.')
  assert.match(refresh, /collection\.discovery\.exhausted/, 'Full snapshots must require proven search exhaustion.')
  assert.match(refresh, /!collection\.discovery\.capped/, 'Full snapshots must fail closed when the collector hits its listing cap.')
  assert.match(refresh, /collection\.failures\.length === 0/, 'Full snapshots must not close missing listings when detail collection failed.')
  assert.match(refresh, /validCoverage >= MIN_VALID_COVERAGE/, 'Full snapshots must enforce minimum valid-row coverage.')
  assert.match(refresh, /p_full_snapshot: fullSnapshot/, 'Removal reconciliation must only run for a verified complete snapshot.')
  assert.match(operational, /portal_current_houses/, 'Active inventory must derive from the canonical live Portal house source.')
