...
async function main() {
  ...
  assert.match(refresh, /status: 'running'/, 'Inventory runs must start non-published while evidence is being persisted.')
  assert.match(refresh, /stage: 'persisting_inventory_presence'/, 'Inventory persistence must be explicitly staged before publication.')
  assert.match(refresh, /status: 'completed'[\s\S]*stage: 'inventory_presence'/, 'Inventory runs may become complete only after evidence persistence succeeds.')
  assert.match(refresh, /INVENTORY_PERSISTENCE_FAILED/, 'Inventory persistence failures must fail closed and remain unpublished.')
  ...
  assert.match(proxy, /pathname === '\/api\/internal\/portal-collector-smoke'/, 'Collector smoke route must reach its own branch guard before session enforcement.')
