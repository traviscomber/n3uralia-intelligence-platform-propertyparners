...
expect(data.sourceInventory?.cellCoverage?.workbookCount === 84, 'Cell coverage must include all 84 workbooks.')
expect(data.sourceInventory?.cellCoverage?.sheetCount === 92, 'Cell coverage must include all 92 sheets.')
expect(data.sourceInventory?.cellCoverage?.storedCells === 1209831, 'Stored-cell coverage must reconcile to 1,209,831 cells.')
expect(data.sourceInventory?.cellCoverage?.populatedCells === 1208765, 'Populated-cell coverage must reconcile to 1,208,765 cells.')
expect(data.sourceInventory?.cellCoverage?.formulaCells === 717, 'Formula-cell coverage must reconcile to 717 cells.')
expect(data.sourceInventory?.datasetCoverage?.length === 13, 'The source catalog must retain all 13 dataset families.')
expect(data.sourceInventory?.workbooks?.every((workbook) => workbook.fileSha256?.length === 64 && workbook.sheets?.every((sheet) => sheet.cellDigest?.length === 64)), 'Every workbook and sheet must retain a SHA-256 digest.')
...
expect(data.ytd?.salesByOffice?.reduce((sum, item) => sum + item.count, 0) === 30, 'Exactly 30 of 34 YTD sales must retain branch attribution.')
expect(data.months?.reduce((sum, month) => sum + month.salesUfByOffice.reduce((officeSum, item) => officeSum + item.value, 0), 0) === 540520, 'Branch-attributed YTD UF must reconcile to UF 540,520.')
...
expect(data.leadSnapshots?.length === 5, 'Lead snapshot coverage must include February-June 2026.')
expect(data.leadSnapshots?.find((snapshot) => snapshot.period === '2026-02')?.active === null, 'Missing February active leads must remain null.')
expect(data.aprilFortnightLeadSnapshot?.staleOver15Rate === null, 'The inconsistent April fortnight stale rate must remain unavailable.')
expect(data.aprilFortnightLeadSnapshot?.classificationCoverage === null, 'The incomplete April fortnight classification rate must remain unavailable.')
expect(data.annualContext2025?.captures === 215, 'The 2025 annual context must retain 215 in-scope captures.')
...
expect(data.targetsContract?.sourceIssueCount === 27, 'Targets must retain all 27 source observations.')
expect(data.targetsContract?.criticalSourceIssueCount === 4, 'Targets must retain all four critical monthly source discrepancies.')
