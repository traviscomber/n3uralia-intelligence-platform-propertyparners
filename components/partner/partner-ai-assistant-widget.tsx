...
export function PartnerAIAssistantWidget() {
  ...
  async function ask(q?: string) {
    ...
    try {
      const response = await fetch('/api/partner/question', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
        ...
        <div className="mb-2 flex w-[420px] max-h-[82vh] flex-col overflow-hidden rounded-2xl border border-neutral-800 bg-neutral-950 text-white shadow-2xl">
          ...
          <div className="flex shrink-0 items-center justify-between border-b border-neutral-800 px-5 py-4">
            <div className="flex items-center gap-3">
              <div className="flex h-8 w-8 items-center justify-center rounded-lg border border-green-700/60 bg-green-950/40 text-sm font-bold text-green-400">
                N
              </div>
              <div>
                <p className="text-xs font-semibold uppercase tracking-widest text-green-400">N3uralia</p>
                <p className="text-[11px] text-neutral-400">Copiloto estrategico · Partner</p>
              </div>
            </div>
            <button
          ...
          <div className="flex shrink-0 gap-2 overflow-x-auto px-5 py-3 [scrollbar-width:none]">
            {QUICK_PROMPTS.map((p) => (
              <button
                ...
                className="shrink-0 rounded-full border border-neutral-700 px-3 py-1 text-[11px] text-neutral-300 transition-colors hover:border-green-700/60 hover:text-orange-300"
              >
                {p}
              </button>
            ))}
          ...
          <div className="flex shrink-0 gap-2 border-t border-neutral-800 px-4 py-3">
            <textarea
              ...
              rows={2}
              className="flex-1 resize-none rounded-xl border border-neutral-700 bg-neutral-900 px-3 py-2 text-sm text-neutral-100 placeholder-neutral-500 focus:border-green-700/60 focus:outline-none"
            />
            <button
              onClick={() => ask()}
              disabled={loading || !question.trim()}
              aria-label="Enviar"
              className="self-end rounded-xl border border-green-700/60 bg-green-950/50 p-2.5 text-orange-300 transition-colors hover:bg-orange-900/40 disabled:opacity-40"
            >
              {loading ? (
      ...
      <button
        ...
        className="flex h-14 w-14 items-center justify-center rounded-2xl border border-green-700/70 bg-neutral-950 text-green-400 shadow-2xl transition-all hover:border-orange-500 hover:bg-neutral-900"
      >
        {open ? (
