...
export function WorkspaceShell({ children, className = '' }: { children: ReactNode; className?: string }) {
  return (
    <main className={`min-h-screen bg-[var(--n3-black)] px-4 py-4 text-[var(--n3-text-light)] sm:px-6 md:px-8 md:py-6 ${className}`}>
...
export function WorkspaceHeader({
  ...
  return (
    <header className="grid gap-4 border-b border-[var(--n3-line)] pb-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {actions.map((action) => {
            const classes = action.primary
              ? 'inline-flex min-h-10 items-center justify-center gap-2 bg-[var(--n3-accent)] px-4 text-xs font-semibold text-white transition-opacity hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--n3-teal)] disabled:cursor-not-allowed disabled:opacity-40'
              : 'inline-flex min-h-10 items-center justify-center gap-2 border border-[var(--n3-line)] px-3 text-xs font-semibold text-[var(--n3-text-muted)] transition-colors hover:border-[var(--n3-text-muted)] hover:text-[var(--n3-text-light)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--n3-teal)] disabled:cursor-not-allowed disabled:opacity-40'
            if (action.href) {
              return <Link key={`${action.label}-${action.href}`} href={action.href} aria-label={action.ariaLabel} className={classes}>{action.icon}{action.label}</Link>
            }
...
export function MetricStrip({
  ...
  return (
    <section aria-label="Indicadores" className="grid border-b border-[var(--n3-line)] sm:grid-cols-2 lg:grid-cols-4">
      {items.map((item, index) => (
        <article key={item.label} className={`border-b border-[var(--n3-line)] py-4 ${index % 4 ? 'sm:px-4' : 'pr-4'} ${index < items.length - 1 ? 'lg:border-r' : ''}`}>
...
export function DataStatusBar({
  ...
  return (
    <section aria-label="Estado de los datos" className="mt-5 flex flex-wrap items-center gap-x-6 gap-y-2 border-y border-[var(--n3-line)] py-3 text-xs">
