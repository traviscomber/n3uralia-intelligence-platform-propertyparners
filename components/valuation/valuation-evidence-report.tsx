...
export function ValuationEvidenceReport({ valuationId }: { valuationId: string }) {
  ...
  return <main className="mx-auto max-w-6xl bg-white p-4 text-neutral-900 sm:p-8 print:max-w-none print:p-0">
    ...
    <section aria-labelledby="ficha" className="mt-7 break-inside-avoid"><h2 id="ficha" className="text-xl font-semibold">2. Ficha del inmueble</h2><dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-3">{[
      ['Superficie útil', valuation.useful_area_m2 == null ? 'No disponible' : `${number.format(valuation.useful_area_m2)} m²`],
      ['Superficie construida', valuation.built_area_m2 == null ? 'No disponible' : `${number.format(valuation.built_area_m2)} m²`],
      ['Terreno', valuation.land_area_m2 == null ? 'No disponible' : `${number.format(valuation.land_area_m2)} m²`],
      ['Dormitorios', valuation.bedrooms ?? 'No disponible'],
      ['Baños', valuation.bathrooms ?? 'No disponible'],
      ['Estacionamientos', valuation.parking_spaces ?? 'No disponible'],
    ].map(([label, value]) => <div key={label} className="border border-neutral-300 p-3"><dt className="font-semibold">{label}</dt><dd className="mt-1 text-neutral-700">{String(value)}</dd></div>)}</dl></section>

    <section aria-labelledby="comparables" className="mt-7"><h2 id="comparables" className="text-xl font-semibold">3. Comparables y evidencia</h2><p className="mt-2 text-sm text-neutral-600">Cada registro conserva fuente, fecha observada, metodología, ajuste y decisión. Una publicación no equivale a una venta confirmada.</p>
      <div className="mt-4 overflow-x-auto"><table className="w-full min-w-[880px] border-collapse text-left text-xs"><caption className="sr-only">Comparables utilizados y excluidos en la valorización</caption><thead><tr className="border-y-2 border-neutral-900"><th scope="col" className="p-2">#</th><th scope="col" className="p-2">Fuente y evidencia</th><th scope="col" className="p-2">Propiedad</th><th scope="col" className="p-2">Precio</th><th scope="col" className="p-2">Ajuste</th><th scope="col" className="p-2">Resultado</th></tr></thead><tbody>{data.comparables.map((item) => {
        const evidence = Array.isArray(item.evidence) ? item.evidence[0] : null
        const identity = evidence && typeof evidence === 'object' ? String(evidence.identityStatus || '') : ''
        return <tr key={item.id} className="break-inside-avoid border-b border-neutral-300 align-top"><td className="p-2">{item.rank}</td><td className="p-2"><p className="font-semibold">{item.source_type || 'Fuente no informada'}</p><p className="mt-1 break-all text-neutral-600">{item.source_reference || 'Sin referencia'}</p><p className="mt-1">Observado: {date(item.source_observed_at)}</p><p>Metodología: {item.source_methodology_version || 'No disponible'}</p>{identity ? <p>Identidad: {identity === 'unconfirmed' ? 'operativa no confirmada' : identity}</p> : null}</td><td className="p-2"><p>{item.address || 'Sin dirección'}</p><p className="text-neutral-600">{item.neighborhood || 'Sin barrio'} · {item.property_type || 'Sin tipo'}</p><p className="mt-1">Similitud: {item.similarity_score == null ? 'No disponible' : `${number.format(item.similarity_score)}%`}</p></td><td className="p-2"><p>{uf(item.price_uf)}</p><p>{item.price_uf_m2 == null ? 'UF/m² no disponible' : `${number.format(item.price_uf_m2)} UF/m²`}</p><p>Ajustado: {uf(item.adjusted_value_uf)}</p></td><td className="p-2"><p>{item.adjustment_pct == null ? '0%' : `${number.format(item.adjustment_pct)}%`}</p><p className="mt-1 text-neutral-600">{item.adjustment_notes || item.exclusion_reason || 'Sin observación'}</p></td><td className="p-2"><p className="font-semibold">{item.selected && item.match_status === 'accepted' ? 'Aceptado' : item.match_status === 'excluded' ? 'Excluido' : 'Candidato'}</p></td></tr>
      })}</tbody></table></div>
