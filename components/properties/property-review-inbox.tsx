...
function statusLabel(row:PropertyReviewRow){
  if(row.can_decide&&row.proposed_neighborhood_name)return'Coordenada confirmable'
  if(row.proposed_neighborhood_name)return'Señal secundaria'
    ...
    <section className="min-w-0 p-5 lg:p-7">
      {selected?<div className="mx-auto max-w-4xl">
        ...
        <section className="mt-7 border-t border-[var(--n3-line)] pt-5">
          ...
          {!selected.can_decide?<div className="mt-4 flex items-start gap-2 text-xs leading-5 text-[var(--n3-text-muted)]"><ShieldCheck size={14} className="mt-0.5 shrink-0"/>Esta señal es sólo de apoyo. Memoria, texto, CBRS, reglas, patrones aprendidos y puntos cercanos no pueden confirmar el barrio por sí solos. El caso queda abierto hasta contar con coordenada Portal dentro de un único KML o evidencia territorial validada.</div>:null}
