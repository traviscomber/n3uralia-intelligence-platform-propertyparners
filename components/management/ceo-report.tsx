...
export function CeoReport(){
  ...
  const pending=operations?operations.tasks.overdue+operations.valuations.review+operations.assignments.paused+operations.market.pendingIdentity:0
  return <IntelligencePage>
    <div className="print-hidden flex flex-wrap items-center justify-between gap-3"><Link href="/dashboard/ceo" className="border border-[var(--n3-line)] px-4 py-2 text-xs focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2">Volver al CEO</Link><PrintReportButton/></div>
    ...
    {summary&&operations&&!error?<>
      ...
      <section className="grid gap-5 xl:grid-cols-2"><div><SectionHeading eyebrow="03 · Top ejecutivas" title="Desempeño por cierres"/><div className="divide-y divide-[var(--n3-line)] border border-[var(--n3-line)]">{top.length?top.map((entity,index)=><div key={entity.id} className="flex items-center justify-between p-4"><div><span className="mr-3 text-xs text-[var(--n3-text-muted)]">{index+1}</span><strong>{entity.name}</strong></div><span>{fmt(metric(entity,'sales'))}</span></div>):<p className="p-5 text-sm text-[var(--n3-text-muted)]">Sin datos suficientes para ranking.</p>}</div></div><div><SectionHeading eyebrow="04 · Decisiones" title={pending?'Pendientes activos':'Sin pendientes críticos'}/><div className="space-y-3"><article className="border border-[var(--n3-line)] p-4">{operations.tasks.overdue} tareas vencidas · {operations.tasks.urgent} urgentes</article><article className="border border-[var(--n3-line)] p-4">{operations.valuations.review} valorizaciones en revisión</article><article className="border border-[var(--n3-line)] p-4">{operations.assignments.paused} asignaciones pausadas</article><article className="border border-[var(--n3-line)] p-4">{operations.market.pendingIdentity} identidades de mercado pendientes</article></div></div></section>
      <section><SectionHeading eyebrow="05 · Seguimiento" title="Tareas prioritarias"/><div className="space-y-3">{priorityTasks.length?priorityTasks.map((task)=><article key={task.id} className="border border-[var(--n3-line)] p-4"><p className="font-semibold">{task.title}</p><p className="mt-1 text-xs text-[var(--n3-text-muted)]">{task.office??'Sin oficina'} · {task.assignedProfile?.full_name??'Sin responsable'} · vence {task.due_date??'sin fecha'}</p></article>):<p className="border border-dashed border-[var(--n3-line)] p-5 text-sm text-[var(--n3-text-muted)]">No existen tareas urgentes o vencidas al momento de generar el reporte.</p>}</div></section>
