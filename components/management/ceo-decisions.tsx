...
export function CeoDecisions(){
  ...
  return <IntelligencePage>
    ...
    <IntelligenceHeader eyebrow="CEO · Centro de decisiones" title="Qué requiere atención ahora" description="Una sola vista para decisiones abiertas, seguimiento activo y resultados recientes. La evidencia permanece disponible sin competir por atención." actions={[{label:'Valorizaciones',href:'/dashboard/valuations',primary:true},{label:'Propiedades',href:'/dashboard/properties/admin'}]}/>
        ...
        <div className="space-y-3">{following.length?following.map((task)=>{
          ...
          return <article key={task.id} className={`border bg-[#0c1111] p-4 ${taskBorder(task,today)}`}>
            <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
              <div className="min-w-0 flex-1"><p className="font-semibold">{task.title}</p><p className="mt-1 text-xs text-[var(--n3-text-muted)]">{task.office??'Sin oficina'} · {task.assignedProfile?.full_name??'Sin responsable'} · vence {formatDate(task.due_date)}</p>{task.detail?<p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--n3-text-muted)]">{task.detail}</p>:null}</div>
