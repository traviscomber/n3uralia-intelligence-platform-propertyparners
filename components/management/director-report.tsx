...
export function DirectorReport() {
  ...
  return <IntelligencePage>
    <div className="print-hidden flex flex-wrap items-center justify-between gap-3"><Link href="/dashboard/director" className="border border-[var(--n3-line)] px-4 py-2 text-xs">Volver a dirección</Link><PrintReportButton/></div>
    <IntelligenceHeader eyebrow="Reporte de dirección" title={`Oficina ${payload?.scopeLabel ?? ''}`} description="Resultado comercial, equipo, alertas y operación bajo el alcance autorizado." meta={<div className="text-xs text-[var(--n3-text-muted)]">ID {generatedId}</div>} />
    ...
    {payload ? <>
      <section><SectionHeading eyebrow="01 · Resumen" title="Resultado y operación"/><MetricGrid columns={4}><MetricCard label="Cierres junio" value={fmt(getMetric(branch,'sales'))} detail={`Meta ${getMetric(branch,'sales')?.target ?? 'n/d'}`}/><MetricCard label="Venta junio" value={fmt(getMetric(branch,'sales_uf'))}/><MetricCard label="Valorizaciones en revisión" value={payload.operational?.valuationInReview ?? 0}/><MetricCard label="Propiedades activas" value={payload.operational?.activePropertyAssignments ?? 0}/></MetricGrid></section>
