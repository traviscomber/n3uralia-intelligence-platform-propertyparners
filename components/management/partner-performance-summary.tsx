...
export async function PartnerPerformanceSummary() {
  ...
  const partner = canonical.partners.find((item) => {
  ...
  if (!partner) {
    return (
      <section className="mx-auto mt-8 max-w-7xl border border-dashed border-[var(--n3-line)] p-6 text-sm text-[var(--n3-text-muted)]">
        No existe una ficha canónica vinculada de forma inequívoca a este perfil. No se presentan métricas inferidas.
      </section>
    )
  }

  const annual = parseCanonicalSalesComparison(partner)
  ...
  return (
    <section className="mx-auto mt-8 max-w-7xl space-y-5">
      <div className="border-b border-[var(--n3-line)] pb-4">
        <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#ff766f]">Lectura contractual personal</p>
        <h2 className="mt-2 text-2xl font-semibold">Metas, evolución y calidad comercial</h2>
        <p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--n3-text-muted)]">Una sola lectura personal con valores, metas, comparaciones y procedencia visibles. No mezcla métricas de otras ejecutivas.</p>
      </div>
      <div className="grid gap-px bg-[var(--n3-line)] sm:grid-cols-2 xl:grid-cols-3">
        {cards.map(([label, value, detail]) => (
          <article key={label} className="bg-[#0c1111] p-5">
      ...
      <div className="border-l-2 border-[#d7332b] pl-4 text-xs leading-5 text-[var(--n3-text-muted)]">
        Fuente: {source.deck} · lámina {source.slide} · {source.title}. Período principal: junio de 2026; acumulado enero–junio de 2026. Comparación YoY contra 2025 cuando la tabla canónica contiene base explícita.
      </div>
    </section>
  )
}
