  ...
  return <IntelligencePage>
    ...
    {!loading&&!error&&summary&&operations?<>
      ...
      <section><SectionHeading eyebrow="02 · Indicadores exigidos" title="Cobertura del Dashboard CEO" description="Los datos ausentes se muestran como n/d; no se infieren desde indicadores distintos."/><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <MetricCard label="Ventas (cierres)" value={fmt(companySales)} detail={`Meta ${companySales?.target??'n/d'} · ${compliance(companySales)}`}/>
        ...
        <MetricCard label="Variación MoM" value={pct(companySales?.mom)} detail="Cierres contra el mes anterior disponible"/>
        <MetricCard label="Variación YoY" value={pct(companySales?.yoy)} detail={companySales?.yoy==null?'Sin base comparable del año anterior':'Cierres contra igual período anterior'}/>
      ...
      <MethodologyNote>{summary.dataProvenance} Generado {new Date(operations.generatedAt).toLocaleString('es-CL')}. Captaciones y YoY permanecen n/d cuando la fuente no los entrega. Productividad es un proxy sobre fichas canónicas visibles, no la dotación total. Los consolidados y las oficinas se reproducen desde sus fuentes respectivas y no se fuerzan a cuadrar por suma. Las alertas usan reglas operativas derivadas, no indicadores canónicos adicionales.</MethodologyNote>
    </>:null}
  </IntelligencePage>
}
