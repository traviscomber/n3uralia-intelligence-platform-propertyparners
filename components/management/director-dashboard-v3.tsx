...
type Task = { id:string; source_key:string|null; title:string; detail:string|null; severity:string; status:string; due_date:string|null; created_at:string }
...
export function DirectorDashboardV3() {
  ...
  async function createTask(alert:Alert){
    ...
    try{
      ...
      const response=await fetch('/api/management/tasks',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({sourceKey:alert.id,title:`${alert.entityName} · ${alert.title}`,detail:alert.detail,severity:alert.severity,entityName:alert.entityName,dueDate:due.toISOString().slice(0,10)})})
  ...
  return <IntelligencePage>
    ...
    {loading?<div className="border border-[var(--n3-line)] p-8 text-sm text-[var(--n3-text-muted)]">Cargando indicadores, alertas y tareas…</div>:null}
