...
export function ExecutiveDecisionGraphView({ graph }: Props) {
  ...
  function selectCase(caseId: string, target?: 'timeline' | 'simulator') {
    setSelectedId(caseId)
    if (target) {
      window.requestAnimationFrame(() => {
        document
          .getElementById(target === 'timeline' ? 'executive-evidence-timeline' : 'executive-impact-simulator')
      ...
      <div className="border border-[var(--n3-line)] bg-[#0c1111]">
        <div className="flex flex-col gap-4 border-b border-[var(--n3-line)] p-5 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-[var(--n3-text-muted)]">Selecciona un caso para inspeccionar conexiones, evidencia e impacto potencial.</p>
          </div>
