...
alter table private.market_listing_identity_memory_v1
  alter column decided_by drop not null;

alter table private.market_live_identity_decisions_v1
  alter column decided_by drop not null;

alter table private.market_listing_identity_memory_v1
  add column if not exists decision_origin text not null default 'human'
  check (decision_origin in ('human','system_high_confidence'));
...
drop trigger if exists trg_auto_resolve_high_confidence_live_identity_v1 on public.market_property_matches;
create trigger trg_auto_resolve_high_confidence_live_identity_v1
after insert or update of status, score, evidence, contradictions
on public.market_property_matches
for each row
execute function private.auto_resolve_high_confidence_live_identity_v1();

-- Process any already-generated high-confidence candidates under the same policy.
update public.market_property_matches
set score=score
where left_entity_type='listing'
  and right_entity_type='property'
  and status='candidate_high'
  and score>=0.94;
