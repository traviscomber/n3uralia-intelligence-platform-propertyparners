...
select f.id,f.event_key,f.transaction_date,f.address,f.rol,f.price_uf,f.built_area_m2,f.land_area_m2,f.construction_year,f.bedrooms_bathrooms,f.latitude,f.longitude,f.neighborhood,f.barrio_nombre,
 jsonb_build_object(
   ...
   'cohortRatioOutlier',coalesce(f.ratio_to_median<0.55 or f.ratio_to_median>2.0,false),
   'rolRatioOutlier',coalesce(f.transaction_count>=2 and f.price_ratio>=3,false),
   'priceOutlier',coalesce(f.canonical_uf_m2<f.lower_fence or f.canonical_uf_m2>f.upper_fence or f.ratio_to_median<0.55 or f.ratio_to_median>2.0 or (f.transaction_count>=2 and f.price_ratio>=3),false),
   'policy','cbrs_quality_v7_pp_kml_robust_cohort'
 )
