...
begin
  ...
  v_baseline := coalesce(
    nullif(new.evidence->>'recommendedRateUfM2', '')::numeric,
  ...
  v_prediction := public.valuation_ml_house_shadow_v1(
    ...
    p_strict_comparable_count => coalesce(
      nullif(new.evidence->>'strictComparableCount', '')::integer,
      nullif(new.evidence->>'comparableCount', '')::integer
    ),
    p_average_similarity => nullif(new.evidence->>'averageSimilarity', '')::numeric,
    p_comparable_spread => nullif(new.evidence->>'comparableSpread', '')::numeric,
    p_as_of => now()
  );
  ...
  v_features := coalesce(v_prediction->'features', '{}'::jsonb) || jsonb_build_object(
    ...
    'capturedBy', 'valuation_case_trigger_v2',
    'baselineSource', case when nullif(new.evidence->>'recommendedRateUfM2', '') is not null then 'champion_recommendation' else 'case_rate_fallback' end,
    'valuerConfirmedRateUfM2', new.built_rate_uf_m2,
    'recommendedRateUfM2', nullif(new.evidence->>'recommendedRateUfM2', '')::numeric
  );
