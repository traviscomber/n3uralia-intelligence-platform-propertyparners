...
begin
  ...
  if v_event_key is not null then
    select * into v_ref from public.market_cbrs_reference_transactions r where r.event_key = v_event_key limit 1;
  end if;
  if v_ref.id is null and v_rol is not null then
    select * into v_ref from public.market_cbrs_reference_transactions r
    where r.rol = v_rol and (p_price_uf is null or r.price_uf = p_price_uf)
  ...
  if v_ref.id is not null and nullif(trim(coalesce(v_kml_neighborhood, '')), '') is not null then
    ...
    where rr.price_uf > 0
      ...
      and private.resolve_vitacura_kml_neighborhood_v1(rr.latitude, rr.longitude) = v_kml_neighborhood
  ...
  return jsonb_build_object(
    ...
    'transactionCount', coalesce(v_transaction_count,0),
    'minUf', v_min_uf,
    'maxUf', v_max_uf,
    'priceRatio', v_price_ratio,
    'sameRolPriceConflict', coalesce(v_same_rol_conflict,false),
    ...
    'policy', 'cbrs_quality_v3_pp_kml_house_weighted_area'
  );
end;
$$;

-- Recreate the KML candidate RPC with quality evidence and hard exclusion of same-ROL conflicts.
...
as $$
  with eligible as (
    ...
    where lower(coalesce(t.property_type,''))=lower(coalesce(p_property_type,''))
      and coalesce(t.price_uf,0)>0 and coalesce(t.built_area_m2,0)>0
      and (lower(coalesce(p_property_type,''))<>'casa' or coalesce(t.land_area_m2,0)>0)
  ), scored as (
    select e.*, private.cbrs_reference_quality_v2('CBRS '||e.event_key||case when e.rol is null then '' else ' · ROL '||e.rol end,e.price_uf,e.canonical_uf_m2,e.barrio_nombre,p_property_type) as q
    from eligible e
  )
  select s.id,s.event_key,s.transaction_date,s.address,s.rol,s.price_uf,s.built_area_m2,s.land_area_m2,s.latitude,s.longitude,s.neighborhood,s.barrio_nombre,s.q
  from scored s
  where coalesce((s.q->>'sameRolPriceConflict')::boolean,false)=false
    and coalesce((s.q->>'geographyConflict')::boolean,false)=false
  order by s.transaction_date desc nulls last,s.id
