...
begin
  ...
  portal as (
    select
      ...
      (percentile_cont(0.5) within group (order by r.price_uf_m2)
        filter (where r.price_uf_m2 > 0))::numeric as portal_median_uf_m2,
...
comment on function public.get_market_house_supply_sales_live_v1() is
  'Authenticated live house supply vs CBRS metrics by canonical Property Partners KML neighborhood. Exposes observed metrics only; it does not classify price signals.';
