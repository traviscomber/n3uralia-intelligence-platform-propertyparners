create or replace view private.market_production_release_gate_v1
...
property_quality as (
  select
    ...
    count(*) filter (where identity_status not in ('candidate','confirmed'))::bigint as unexpected_identity_status_rows,
  ...
  where q.identity_signal_class = 'probable_duplicate_listing'
    and not exists (
      ...
      join public.market_property_matches m
       ...
       and (
         (m.left_entity_type = 'property' and m.right_entity_type = 'property'
          and m.left_entity_id = q.property_id and m.right_entity_id = q2.property_id)
         or
         (m.left_entity_type = 'property' and m.right_entity_type = 'property'
          and m.left_entity_id = q2.property_id and m.right_entity_id = q.property_id)
       )
...
neighborhood_quality as (
  select
    count(*)::bigint as neighborhood_rows,
...
assembled as (
  select
    ...
    pq.*,
    ei.*,
    ud.rows_without_confirmed_duplicate_edge,
    nq.neighborhood_rows,
    nq.neighborhood_rows_missing_source_url,
    cc.transaction_rows,
    cc.valuation_case_rows,
    cc.valuation_comparable_rows,
    cc.management_approved_rows,
    case when pq.latest_property_seen_at is null then null
         else current_date - pq.latest_property_seen_at::date end as market_age_days
  from raw_quality rq
...
classified as (
  select a.*,
    array_remove(array[
      ...
      case when a.neighborhood_rows > 0 and a.neighborhood_rows_missing_source_url > 0 then 'neighborhood_provenance_missing' end
    ], null) as blockers,
    array_remove(array[
...
select
  ...
  market_age_days,
  neighborhood_rows,
  neighborhood_rows_missing_source_url,
  transaction_rows,
  valuation_case_rows,
...
comment on view private.market_production_release_gate_v1 is 'Server-only production release gate for market data integrity. BLOCK means production release is unsafe; HOLD means core integrity blockers are absent but dependent capabilities must remain disabled; PASS requires all declared gates and capabilities.';
