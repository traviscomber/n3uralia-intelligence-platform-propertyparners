...
select distinct trim(n.name), null::text, null::jsonb, null::uuid,
...
with combined as (
  select
    ...
    max(md.inventory_count) as active_inventory,
    max(md.absorption_rate) as absorption_rate,
    max(md.avg_days_on_market) as median_days_on_market
  from market_data md
