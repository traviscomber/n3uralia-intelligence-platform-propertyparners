-- Make valuation comparables geography-aware using Property Partners' canonical KML.
-- Prefer point-in-polygon barrio evidence when coordinates exist and deduplicate repeated portal ads.
...
with subject as (
  select
    vc.*,
    coalesce(
      geo_subject.neighborhood_id,
      mp.neighborhood_id,
      (
        select mn0.id from public.market_neighborhoods mn0
  ...
  left join lateral (
    select mn.id as neighborhood_id
    from public.vitacura_market_neighborhoods vmn
    join public.market_neighborhoods mn
      on lower(extensions.unaccent(mn.name))=lower(extensions.unaccent(vmn.barrio_nombre))
    where vc.latitude is not null and vc.longitude is not null
      and st_contains(vmn.geometry,st_setsrid(st_makepoint(vc.longitude::double precision,vc.latitude::double precision),4326))
    limit 1
  ) geo_subject on true
...
latest_listings as (
  select distinct on (
    coalesce(private.normalize_market_external_id(coalesce(ml.url,ml.source_listing_id)),ml.source_id::text||':'||ml.source_listing_id)
  ) ml.*
  ...
  order by
    coalesce(private.normalize_market_external_id(coalesce(ml.url,ml.source_listing_id)),ml.source_id::text||':'||ml.source_listing_id),
    ml.observed_at desc,ml.created_at desc
),
raw_candidates as (
  select
    ...
    coalesce(geo_tx.neighborhood_name,mn.name) as neighborhood,mp.property_type,
    ...
    case when s.latitude is not null and s.longitude is not null and mp.latitude is not null and mp.longitude is not null
      then 111320*sqrt(power((mp.latitude-s.latitude)::numeric,2)+power(((mp.longitude-s.longitude)*cos(radians(s.latitude::double precision)))::numeric,2)) else null end as distance_meters,
    ...
    jsonb_build_array(jsonb_build_object(
      'marketSourceId',mt.source_id,'eventKey',mt.event_key,'transactionDate',mt.transaction_date,
      'priceUf',mt.price_uf,'priceUfM2',mt.price_uf_m2,'scopeGuard','canonical_transaction',
      'geographyMethod',case when geo_tx.neighborhood_id is not null then 'property_partners_kml_point_in_polygon' else 'stored_neighborhood' end
    )) as source_evidence
  from subject s
  ...
  left join lateral (
    select mn2.id as neighborhood_id,mn2.name as neighborhood_name
    from public.vitacura_market_neighborhoods vmn2
    join public.market_neighborhoods mn2
      on lower(extensions.unaccent(mn2.name))=lower(extensions.unaccent(vmn2.barrio_nombre))
    where mp.latitude is not null and mp.longitude is not null
      and st_contains(vmn2.geometry,st_setsrid(st_makepoint(mp.longitude::double precision,mp.latitude::double precision),4326))
    limit 1
  ) geo_tx on true
  ...
  select
    ...
    null::date,ml.observed_at,coalesce(ml.normalized_address,mp.normalized_address),
    coalesce(geo_listing.neighborhood_name,mn.name),mp.property_type,
    mp.useful_area_m2,mp.built_area_m2,mp.land_area_m2,mp.bedrooms,mp.bathrooms,mp.parking_spaces,
    ml.price_uf,ml.price_uf_m2,
    ...
    coalesce(geo_listing.neighborhood_id,mp.neighborhood_id),
    coalesce(mp.useful_area_m2,mp.built_area_m2),s.comparison_area_m2,s.resolved_neighborhood_id,s.bedrooms,s.bathrooms,
    jsonb_build_array(jsonb_build_object(
      'marketSourceId',ml.source_id,'listingId',ml.source_listing_id,'url',ml.url,'observedAt',ml.observed_at,
      'status',ml.status,'priceUf',ml.price_uf,'priceUfM2',ml.price_uf_m2,'scopeGuard','latest_listing_deduplicated_v4',
      'geographyMethod',case when geo_listing.neighborhood_id is not null then 'property_partners_kml_point_in_polygon' else 'stored_neighborhood' end
    ))
  from subject s
  ...
  left join public.market_neighborhoods mn on mn.id=mp.neighborhood_id
  left join lateral (
    select mn3.id as neighborhood_id,mn3.name as neighborhood_name
    from public.vitacura_market_neighborhoods vmn3
    join public.market_neighborhoods mn3
      on lower(extensions.unaccent(mn3.name))=lower(extensions.unaccent(vmn3.barrio_nombre))
    where ml.latitude is not null and ml.longitude is not null
      and st_contains(vmn3.geometry,st_setsrid(st_makepoint(ml.longitude::double precision,ml.latitude::double precision),4326))
    limit 1
  ) geo_listing on true
  where lower(coalesce(mp.property_type,''))=lower(coalesce(s.property_type,''))
    and (s.subject_property_id is null or mp.id<>s.subject_property_id)
...
select
  ...
  s.transaction_date,s.observed_at,s.address,s.neighborhood,s.property_type,s.useful_area_m2,s.built_area_m2,s.land_area_m2,
  s.bedrooms,s.bathrooms,s.parking_spaces,s.price_uf,s.price_uf_m2,s.distance_meters,
  greatest(0.1,least(1,s.final_similarity)) as similarity_score,
  s.source_evidence || jsonb_build_array(jsonb_build_object(
    'methodology','valuation_candidate_pool_v4_kml','evaluatedWeight',s.evaluated_weight,
