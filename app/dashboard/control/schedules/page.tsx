...
export default function ManagementSchedulesPage() {
  ...
  async function load() {
    ...
    if (response.ok) { setSchedules(data.schedules ?? []); setEntities(data.entities ?? []); setReadiness(data.automationReadiness ?? initialReadiness) } else setMessage(data.error ?? 'No fue posible cargar la programación.')
  }
  useEffect(() => { void load() }, [])

  async function save(event:React.FormEvent) {
    event.preventDefault(); setMessage('')
    if (!readiness.ready) { setMessage('Programación bloqueada hasta aprobar el diccionario KPI y las reglas de reporting.'); return }
  ...
  return <div className="space-y-7">
    ...
    <form onSubmit={save} className="grid gap-4 border border-[var(--n3-line)] p-5 lg:grid-cols-3">
      <label className="text-sm">Nombre<input disabled={!readiness.ready} value={form.name} onChange={e=>setForm({...form,name:e.target.value})} className="mt-2 w-full border border-[var(--n3-line)] bg-transparent p-3 disabled:opacity-40" /></label>
      <label className="text-sm">Tipo<select disabled={!readiness.ready} value={form.reportType} onChange={e=>setForm({...form,reportType:e.target.value})} className="mt-2 w-full border border-[var(--n3-line)] bg-[var(--n3-black)] p-3 disabled:opacity-40"><option value="management">Gestión</option><option value="executive">Ejecutivo</option><option value="director">Dirección</option></select></label>
      <label className="text-sm">Entidad<select disabled={!readiness.ready} value={form.entityId} onChange={e=>setForm({...form,entityId:e.target.value})} className="mt-2 w-full border border-[var(--n3-line)] bg-[var(--n3-black)] p-3 disabled:opacity-40"><option value="">Consolidado</option>{entities.map(entity=><option key={entity.id} value={entity.id}>{entity.name} · {entity.entity_type}</option>)}</select></label>
      <label className="text-sm">Frecuencia<select disabled={!readiness.ready} value={form.cadence} onChange={e=>setForm({...form,cadence:e.target.value})} className="mt-2 w-full border border-[var(--n3-line)] bg-[var(--n3-black)] p-3 disabled:opacity-40"><option value="monthly">Mensual</option></select></label>
      <label className="text-sm">Día del mes<input disabled={!readiness.ready} type="number" min="1" max="28" value={form.dayOfMonth} onChange={e=>setForm({...form,dayOfMonth:e.target.value})} className="mt-2 w-full border border-[var(--n3-line)] bg-transparent p-3 disabled:opacity-40" /></label>
      <label className="text-sm">Destinatarios<input disabled={!readiness.ready} type="email" multiple value={form.recipients} onChange={e=>setForm({...form,recipients:e.target.value})} placeholder="correo1@dominio.cl; correo2@dominio.cl" className="mt-2 w-full border border-[var(--n3-line)] bg-transparent p-3 disabled:opacity-40" /></label>
      <div className="lg:col-span-3"><button disabled={!readiness.ready} className="border border-[#d7332b] px-5 py-3 text-sm disabled:cursor-not-allowed disabled:opacity-40">Crear programación</button></div>
    </form>
