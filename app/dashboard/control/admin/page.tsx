...
export default function ManagementAdminPage() {
  ...
  async function post(body: unknown) {
    ...
    try {
      ...
      setFeedback({ kind: 'success', message: 'Cambios guardados correctamente.' })
      await load()
    } catch {
  ...
  return (
    <IntelligencePage>
      ...
      {feedback?.kind === 'success' ? <div role="status" className="border border-[var(--n3-teal)] bg-[#0c1111] p-4 text-sm text-[var(--n3-text-light)]">{feedback.message}</div> : null}
      {loading ? <div role="status" aria-live="polite" className="border border-[var(--n3-line)] p-8 text-sm text-[var(--n3-text-muted)]">Cargando…</div> : null}
      ...
      {!loading && data ? <>
        <section>
          ...
          <IntelligencePanel eyebrow="Registro" title="Crear o actualizar meta" description="La combinación entidad, métrica y período se actualiza sin duplicados.">
            <div className="grid gap-3 p-5 md:grid-cols-2 xl:grid-cols-5">
              ...
              <button disabled={saving} onClick={() => { const bounds = monthBounds(goal.month); void post({ type: 'goal', ...goal, periodStart: bounds.start, periodEnd: bounds.end }) }} className="flex items-center justify-center gap-2 border border-[#d7332b] p-3 disabled:cursor-not-allowed disabled:opacity-50"><Save size={15} />{saving ? 'Guardando…' : 'Guardar meta'}</button>
            </div>
          </IntelligencePanel>
        ...
        <section>
          ...
          <IntelligencePanel eyebrow="Dato operativo" title="Carga manual no publicada" description="La carga queda como evidencia independiente. No alimenta dashboards hasta ser comparada con el cálculo canónico y aprobada por CEO.">
            <div className="grid gap-3 p-5 md:grid-cols-2 xl:grid-cols-4">
              ...
              <button disabled={saving} onClick={() => { const bounds = monthBounds(metric.month); void post({ type: 'metric', ...metric, periodStart: bounds.start, periodEnd: bounds.end }) }} className="flex items-center justify-center gap-2 border border-[#d7332b] p-3 disabled:cursor-not-allowed disabled:opacity-50"><Database size={15} />{saving ? 'Guardando…' : 'Registrar evidencia'}</button>
            </div>
          </IntelligencePanel>
        </section>
        ...
        <section>
          ...
          <IntelligencePanel eyebrow="Configuración CEO" title="Crear o actualizar regla" description="El código identifica la regla y permite actualizarla sin perder trazabilidad.">
            <div className="grid gap-3 p-5 md:grid-cols-2 xl:grid-cols-4">
              ...
              <button disabled={saving} onClick={() => void post({ type: 'rule', ...rule })} className="flex items-center justify-center gap-2 border border-[#d7332b] p-3 disabled:cursor-not-allowed disabled:opacity-50"><Target size={15} />{saving ? 'Guardando…' : 'Guardar regla'}</button>
            </div>
          </IntelligencePanel>
        </section>
        ...
        <section>
          ...
          <div className="space-y-3">{openAlerts.length ? openAlerts.map((alert) => <div key={alert.id} className={`border p-4 ${alert.severity === 'critical' ? 'border-[#d7332b]' : 'border-[var(--n3-line)]'}`}><div className="flex flex-col justify-between gap-4 md:flex-row"><div className="flex gap-3"><AlertTriangle size={18} className="mt-0.5 text-[#ff766f]" /><div><p className="font-semibold">{alert.title}</p><p className="mt-1 text-xs text-[var(--n3-text-muted)]">{alert.management_entities?.name ?? 'Entidad'} · {alert.severity} · {alert.status}</p><p className="mt-2 text-sm text-[var(--n3-text-muted)]">{alert.detail}</p></div></div><div className="flex gap-2"><button disabled={saving} onClick={() => void updateAlert(alert.id, 'acknowledge')} className="border border-[var(--n3-line)] px-3 py-2 text-xs disabled:opacity-50">Reconocer</button><button disabled={saving} onClick={() => void updateAlert(alert.id, 'resolve')} className="border border-[var(--n3-line)] px-3 py-2 text-xs disabled:opacity-50">Resolver</button><button disabled={saving} onClick={() => void updateAlert(alert.id, 'dismiss')} className="border border-[var(--n3-line)] px-3 py-2 text-xs disabled:opacity-50">Descartar</button></div></div></div>) : <div className="border border-dashed border-[var(--n3-line)] p-6 text-sm text-[var(--n3-text-muted)]">No hay alertas abiertas.</div>}</div>
        </section>
