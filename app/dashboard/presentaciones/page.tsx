  ...
  return (
    <div className="mx-auto max-w-[1600px] space-y-6 pb-16 text-[var(--n3-text-light)]">
      <header className="overflow-hidden border border-white/10 bg-black text-white">
        <div className="grid gap-8 p-7 lg:grid-cols-[minmax(0,1fr)_420px] lg:p-10">
          <div>
            ...
            <p className="mt-8 text-[11px] font-semibold uppercase tracking-[0.26em] text-[var(--n3-teal)]">Control de gestión · fuente real</p>
            ...
            <p className="mt-3 max-w-3xl text-sm leading-6 text-[var(--n3-text-muted)]">Cada cifra de los cinco PPTX se conserva como snapshot de gestión y se compara con CRM y Metas 2026. Las diferencias se explicitan; no se corrigen, promedian ni ocultan.</p>
          </div>
      ...
      <section className="grid gap-px bg-[var(--n3-line)] lg:grid-cols-3">
        ...
        ].map(([eyebrow, title, text], index) => <article key={title} className="bg-[var(--n3-deep)] p-5"><p className={`text-[10px] font-bold uppercase tracking-[0.16em] ${index === 2 ? 'text-[#d7332b]' : 'text-[var(--n3-text-muted)]'}`}>{eyebrow}</p><h2 className="mt-2 text-lg font-semibold">{title}</h2><p className="mt-2 text-xs leading-5 text-[var(--n3-text-muted)]">{text}</p></article>)}
      </section>
      ...
      <section className="grid gap-4 xl:grid-cols-[1.1fr_1.9fr]">
        <article className="border border-[var(--n3-line)] bg-[var(--n3-deep)] p-5">
          ...
          <div className="mt-5 grid grid-cols-2 gap-px bg-[var(--n3-line)]">
            <div className="bg-[var(--n3-deep)] p-4"><p className="text-xs text-[var(--n3-text-muted)]">PPT Directorio</p><p className="mt-1 text-3xl font-semibold">{format(data.reconciliation.headline.presentationYtdSales)}</p><p className="text-xs text-[var(--n3-text-muted)]">{format(data.reconciliation.headline.presentationYtdUf)} UF</p></div>
            <div className="bg-[var(--n3-deep)] p-4"><p className="text-xs text-[var(--n3-text-muted)]">CRM normalizado</p><p className="mt-1 text-3xl font-semibold">{format(data.reconciliation.headline.crmYtdSales)}</p><p className="text-xs text-[var(--n3-text-muted)]">{format(data.reconciliation.headline.crmYtdUf)} UF</p></div>
          </div>
        ...
        <article className="border border-[var(--n3-line)] bg-[var(--n3-deep)] p-5">
          <div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[var(--n3-text-muted)]">Scoring real · junio</p><h2 className="mt-2 text-xl font-semibold">Lectura por sucursal</h2></div><p className="text-xs text-[var(--n3-text-muted)]">Fórmula: 40% cartera · 30% seguimiento · 30% conversión</p></div>
          <div className="mt-5 grid gap-px bg-[var(--n3-line)] md:grid-cols-3">
            {entities.branches.map((branch) => <div key={branch.name} className="bg-[var(--n3-deep)] p-4"><div className="flex items-start justify-between gap-3"><h3 className="font-semibold">{branch.name}</h3><span className="text-2xl font-semibold text-[#d7332b]">{format(branch.scores.management)}</span></div><div className="mt-4 space-y-2 text-xs text-[var(--n3-text-muted)]"><p>Cartera <strong className="float-right text-[#1565c0]">{format(branch.scores.portfolio)}</strong></p><p>Seguimiento <strong className="float-right text-[#f39c12]">{format(branch.scores.followUp)}</strong></p><p>Conversión <strong className="float-right text-[#27ae60]">{format(branch.scores.conversion)}</strong></p></div><p className="mt-4 border-t border-[var(--n3-line)] pt-3 text-[11px] text-[var(--n3-text-muted)]">{branch.scores.classification || 'Sin clasificación'}</p></div>)}
          </div>
        </article>
      </section>

      <section className="border border-[var(--n3-line)] bg-[var(--n3-deep)]">
        <div className="flex flex-wrap items-end justify-between gap-4 border-b border-[var(--n3-line)] p-5"><div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#d7332b]">Matriz de conciliación</p><h2 className="mt-2 text-xl font-semibold">115 comparaciones trazables</h2></div><div className="flex flex-wrap gap-2">{(['different', 'exact', 'not_comparable'] as const).map((status) => <Link key={status} href={`/dashboard/presentaciones?status=${status}&deck=${encodeURIComponent(selectedDeck.file)}&slide=${selectedSlide.index}`} className="border px-3 py-1.5 text-xs font-semibold" style={{ background: requestedStatus === status ? '#000' : '#fff', color: requestedStatus === status ? '#fff' : 'var(--n3-text-light)', borderColor: requestedStatus === status ? '#000' : 'var(--n3-line)' }}>{statusLabel(status)}</Link>)}</div></div>
      ...
      <section className="border border-[var(--n3-line)] bg-[var(--n3-deep)]">
        ...
        <div className="grid lg:grid-cols-[320px_minmax(0,1fr)]">
          ...
          <article className="min-w-0 p-5 lg:p-7">
            ...
            <div className="mt-5 space-y-3">{selectedSlide.texts.filter(Boolean).map((text, index) => <p key={index} className="border-l-2 border-[var(--n3-line)] pl-3 text-sm leading-6 text-[var(--n3-text-muted)]">{text}</p>)}</div>
            {selectedSlide.tables.map((table, tableIndex) => <div key={table.id || tableIndex} className="mt-6 overflow-x-auto"><p className="mb-2 text-[10px] font-bold uppercase tracking-[0.16em] text-[var(--n3-text-muted)]">Tabla {tableIndex + 1}</p><table className="min-w-full border-collapse text-xs"><tbody>{table.value.map((row, rowIndex) => <tr key={rowIndex} className={rowIndex === 0 ? 'bg-black text-white' : 'border-t border-[var(--n3-line)]'}>{row.map((cell, cellIndex) => <td key={cellIndex} className="whitespace-nowrap px-3 py-2">{cell}</td>)}</tr>)}</tbody></table></div>)}
            {selectedSlide.charts.map((chart, chartIndex) => <div key={chart.id || chartIndex} className="mt-6 border border-[var(--n3-line)] bg-[var(--n3-deep)] p-4"><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[var(--n3-text-muted)]">Gráfico {chartIndex + 1} · {chart.value.chartKinds.join(', ')}</p>{chart.value.series.map((series, seriesIndex) => <div key={seriesIndex} className="mt-3"><p className="text-sm font-semibold">{series.name || 'Serie sin nombre'}</p><p className="mt-1 break-words text-xs text-[var(--n3-text-muted)]">{series.categories.map((category, index) => `${category}: ${series.values[index] ?? 'n/d'}`).join(' · ')}</p><p className="mt-1 font-mono text-[10px] text-[var(--n3-text-muted)]">{series.categoryFormula} · {series.valueFormula}</p></div>)}</div>)}
          </article>
        </div>
      </section>

      <footer className="border-l-2 border-[#d7332b] bg-[var(--n3-deep)] p-4 text-xs leading-5 text-[var(--n3-text-muted)]">Cobertura: {data.source.contentCoverage.textObjects.toLocaleString('es-CL')} objetos de texto, {data.source.contentCoverage.tables} tablas, {data.source.contentCoverage.charts} gráficos y {data.source.contentCoverage.embeddedWorkbooks} libros embebidos. SHA-256 del extracto auditado: <span className="font-mono">{data.source.sha256}</span>.</footer>
    </div>
  )
}
