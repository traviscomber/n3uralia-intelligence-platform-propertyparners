...
type Review = {
  ...
  loCurroAdvisory?:{
    ...
    severity?:string
    message?:string
    challengerValueUf?:number
    deltaPct?:number
    segmentConfidence?:string
    segmentEvidenceN?:number
    segmentWinRatePct?:number
  ...
  return <div className="space-y-6 pb-10">
    <header className="border-b border-[var(--n3-line)] pb-5">
      ...
      <div className="mt-2 flex flex-wrap items-end justify-between gap-4">
        <div><h1 className="text-3xl font-semibold text-[var(--n3-text-light)]">Control de calidad del expediente</h1><p className="mt-2 max-w-3xl text-sm text-[var(--n3-text-muted)]">Síntesis de evidencia, dispersión, similitud y señales no vinculantes. El valor oficial no se modifica desde esta vista.</p></div>
        <div className="border border-[var(--n3-line)] bg-[var(--n3-deep)] px-4 py-3 text-right"><p className="text-[10px] uppercase tracking-[0.14em] text-[var(--n3-text-muted)]">Calidad</p><p className="mt-1 text-2xl font-semibold text-[var(--n3-text-light)]">{review.quality.grade}</p><p className="text-xs text-[var(--n3-teal)]">{review.quality.status.replaceAll('_',' ')}</p></div>
      </div>
    </header>
    ...
    <section className="grid gap-6 lg:grid-cols-[1.2fr_.8fr]">
      ...
      <div className={`border p-5 ${highRisk?'border-amber-700/70 bg-amber-950/20':'border-[var(--n3-line)] bg-[var(--n3-deep)]'}`}>
        <div className="flex items-start gap-3">
          {highRisk?<AlertTriangle className="mt-0.5 h-5 w-5 text-amber-300"/>:<ShieldCheck className="mt-0.5 h-5 w-5 text-[var(--n3-teal)]"/>}
          <div><p className="text-xs uppercase tracking-[0.14em] text-[var(--n3-text-muted)]">Advisory no vinculante</p><h2 className="mt-1 text-lg font-semibold">{advisory?.available?'Revisión adicional recomendada':'Sin advisory especial'}</h2></div>
        </div>
        {advisory?.available ? <div className="mt-5 space-y-4">
          <p className="text-sm leading-6 text-[var(--n3-text-muted)]">{advisory.message}</p>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
            <div className="border border-[var(--n3-line)] p-3"><p className="text-[10px] uppercase text-[var(--n3-text-muted)]">Challenger</p><p className="mt-1 text-lg font-semibold">{uf(advisory.challengerValueUf)}</p></div>
            <div className="border border-[var(--n3-line)] p-3"><p className="text-[10px] uppercase text-[var(--n3-text-muted)]">Diferencia</p><p className="mt-1 text-lg font-semibold">{pct(advisory.deltaPct)}</p></div>
            <div className="border border-[var(--n3-line)] p-3"><p className="text-[10px] uppercase text-[var(--n3-text-muted)]">Confianza segmento</p><p className="mt-1 text-lg font-semibold">{advisory.segmentConfidence || '—'}</p></div>
            <div className="border border-[var(--n3-line)] p-3"><p className="text-[10px] uppercase text-[var(--n3-text-muted)]">Evidencia</p><p className="mt-1 text-lg font-semibold">N={advisory.segmentEvidenceN ?? '—'}</p><p className="text-xs text-[var(--n3-text-muted)]">win {pct(advisory.segmentWinRatePct)}</p></div>
          </div>
        </div> : <p className="mt-5 text-sm leading-6 text-[var(--n3-text-muted)]">El expediente se mantiene en el flujo estándar de revisión profesional.</p>}
      </div>
    </section>

    <div className="flex items-start gap-3 border border-emerald-800/60 bg-emerald-950/20 p-4 text-sm text-emerald-100"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0"/><p>Esta capa es de apoyo profesional. <strong>No cambia el valor oficial</strong> ni modifica inteligencia de mercado canónica.</p></div>
  </div>
}
