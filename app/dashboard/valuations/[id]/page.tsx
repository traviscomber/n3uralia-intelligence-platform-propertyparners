  ...
  return <div className="space-y-6 pb-10">
      ...
      <div className="overflow-x-auto"><table className="min-w-[980px] w-full text-left text-sm"><thead className="bg-black/20 text-xs uppercase tracking-wide text-[var(--n3-text-muted)]"><tr><th className="px-4 py-3">Fuente</th><th className="px-4 py-3">Propiedad</th><th className="px-4 py-3">UF</th><th className="px-4 py-3">Similitud</th><th className="px-4 py-3">Evidencia</th><th className="px-4 py-3">Criterio documentado</th><th className="px-4 py-3">Decisión</th></tr></thead><tbody>
        {data.comparables.map(item=>{
          ...
          return <tr key={item.id} className="border-t border-[var(--n3-line)] align-top">
            ...
            <td className="px-4 py-3"><input disabled={!permissions.canEditComparables} value={adjustments[item.id] ?? String(item.adjustment_pct || 0)} onChange={e=>setAdjustments(v=>({...v,[item.id]:e.target.value}))} type="number" min={-35} max={35} step="0.5" aria-label="Ajuste documentado sin repricing automático" className="w-24 border border-[var(--n3-line)] bg-black/20 px-2 py-1 text-[var(--n3-text-light)] disabled:opacity-60"/><textarea disabled={!permissions.canEditComparables} value={notes[item.id] ?? item.adjustment_notes ?? item.exclusion_reason ?? ''} onChange={e=>setNotes(v=>({...v,[item.id]:e.target.value}))} placeholder="Nota o motivo" className="mt-2 block w-48 border border-[var(--n3-line)] bg-black/20 px-2 py-1 text-xs text-[var(--n3-text-light)] disabled:opacity-60"/></td>
            <td className="px-4 py-3">{permissions.canEditComparables ? <div className="flex gap-2"><button onClick={()=>void comparableAction(item.id,'select')} disabled={busy===item.id || blocked || item.similarity_score<=0 || (item.price_uf_m2??0)<=0} className="inline-flex items-center gap-1 border border-emerald-700 px-2 py-1 text-xs text-emerald-300 disabled:opacity-50"><CheckCircle2 className="h-3.5 w-3.5"/>Aceptar</button><button onClick={()=>void comparableAction(item.id,'exclude')} disabled={busy===item.id} className="inline-flex items-center gap-1 border border-red-800 px-2 py-1 text-xs text-red-300 disabled:opacity-50"><XCircle className="h-3.5 w-3.5"/>Excluir</button></div> : <span className="text-xs text-[var(--n3-text-muted)]">Solo lectura</span>}<p className="mt-2 text-xs uppercase text-[var(--n3-text-muted)]">{blocked?'bloqueado':item.match_status}</p></td>
          </tr>
        })}
    ...
    <section className="grid gap-6 lg:grid-cols-2">
      <div className="border border-[var(--n3-line)] bg-[var(--n3-deep)] p-5"><h2 className="text-lg font-semibold text-[var(--n3-text-light)]">Flujo de aprobación</h2><p className="mt-1 text-sm text-[var(--n3-text-muted)]">Preparar, revisar, aprobar y emitir.</p><textarea value={reason} onChange={e=>setReason(e.target.value)} placeholder="Justificación, observación o motivo de devolución" className="mt-4 min-h-24 w-full border border-[var(--n3-line)] bg-black/20 p-3 text-sm text-[var(--n3-text-light)]"/><div className="mt-4 flex flex-wrap gap-2">
        {valuation.status==='draft' && <button onClick={()=>void workflow('review')} disabled={busy!==null || accepted.length<3 || evidenceAlerts.some(item=>item.selected)} className="inline-flex items-center gap-2 bg-[var(--n3-teal)] px-3 py-2 text-sm text-white disabled:opacity-50"><Send className="h-4 w-4"/>Enviar a revisión</button>}
