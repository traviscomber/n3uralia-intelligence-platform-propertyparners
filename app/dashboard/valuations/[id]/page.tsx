...
import { CheckCircle2, XCircle, Send, ShieldCheck, FileCheck2, History } from 'lucide-react'
...
export default function ValuationWorkspacePage(){
  ...
  return <div className="space-y-6 pb-10">
    ...
    <section className="border border-[var(--n3-line)] bg-[var(--n3-deep)]">
      <div className="border-b border-[var(--n3-line)] p-4"><h2 className="text-lg font-semibold text-[var(--n3-text-light)]">Comparables trazables</h2><p className="text-sm text-[var(--n3-text-muted)]">La generación canónica se realiza en el Valorizador, donde Portal, CBRS y metodología Property Partners permanecen separados. Este expediente revisa únicamente la evidencia persistida; el generador legacy quedó deshabilitado.</p></div>
      <div className="overflow-x-auto"><table className="min-w-[920px] w-full text-left text-sm"><thead className="bg-black/20 text-xs uppercase tracking-wide text-[var(--n3-text-muted)]"><tr><th className="px-4 py-3">Fuente</th><th className="px-4 py-3">Propiedad</th><th className="px-4 py-3">UF</th><th className="px-4 py-3">Similitud</th><th className="px-4 py-3">Criterio documentado</th><th className="px-4 py-3">Decisión</th></tr></thead><tbody>
        {data.comparables.map(item=><tr key={item.id} className="border-t border-[var(--n3-line)] align-top"><td className="px-4 py-3"><p className="font-medium text-[var(--n3-text-light)]">{item.source_type || 'Fuente no informada'}</p><p className="max-w-48 break-words text-xs text-[var(--n3-text-muted)]">{item.source_reference || 'Referencia no disponible'}</p></td><td className="px-4 py-3"><p className="text-[var(--n3-text-light)]">{item.address || 'Dirección no disponible'}</p><p className="text-xs text-[var(--n3-text-muted)]">{item.neighborhood || 'Barrio no disponible'} · {item.property_type || 'Tipo no disponible'}</p></td><td className="px-4 py-3"><p className="font-medium text-[var(--n3-text-light)]">{uf(item.price_uf)}</p><p className="text-xs text-[var(--n3-text-muted)]">{item.price_uf_m2==null?'—':`${n1.format(item.price_uf_m2)} UF/m²`}</p></td><td className="px-4 py-3"><p className="text-[var(--n3-text-light)]">{n1.format(item.similarity_score * 100)}%</p><p className="text-xs text-[var(--n3-text-muted)]">{item.distance_meters==null?'Distancia no disponible':`${nf.format(item.distance_meters)} m`}</p></td><td className="px-4 py-3"><input disabled={!permissions.canEditComparables} value={adjustments[item.id] ?? String(item.adjustment_pct || 0)} onChange={e=>setAdjustments(v=>({...v,[item.id]:e.target.value}))} type="number" min={-35} max={35} step="0.5" aria-label="Ajuste documentado sin repricing automático" className="w-24 border border-[var(--n3-line)] bg-black/20 px-2 py-1 text-[var(--n3-text-light)] disabled:opacity-60"/><textarea disabled={!permissions.canEditComparables} value={notes[item.id] ?? item.adjustment_notes ?? item.exclusion_reason ?? ''} onChange={e=>setNotes(v=>({...v,[item.id]:e.target.value}))} placeholder="Nota o motivo" className="mt-2 block w-48 border border-[var(--n3-line)] bg-black/20 px-2 py-1 text-xs text-[var(--n3-text-light)] disabled:opacity-60"/><p className="mt-1 max-w-48 text-[10px] text-[var(--n3-text-muted)]">Este porcentaje queda como evidencia de revisión y no repricia automáticamente la valorización.</p></td><td className="px-4 py-3">{permissions.canEditComparables ? <div className="flex gap-2"><button onClick={()=>void comparableAction(item.id,'select')} disabled={busy===item.id || item.similarity_score<=0 || (item.price_uf_m2??0)<=0} className="inline-flex items-center gap-1 border border-emerald-700 px-2 py-1 text-xs text-emerald-300 disabled:opacity-50"><CheckCircle2 className="h-3.5 w-3.5"/>Aceptar</button><button onClick={()=>void comparableAction(item.id,'exclude')} disabled={busy===item.id} className="inline-flex items-center gap-1 border border-red-800 px-2 py-1 text-xs text-red-300 disabled:opacity-50"><XCircle className="h-3.5 w-3.5"/>Excluir</button></div> : <span className="text-xs text-[var(--n3-text-muted)]">Solo lectura</span>}<p className="mt-2 text-xs uppercase text-[var(--n3-text-muted)]">{item.match_status}</p></td></tr>)}
