...
export default async function AudiencePage({ params, searchParams }: { params: Promise<{ audience: string }>; searchParams: Promise<{ branch?: string }> }) {
  ...
  const { data: profile } = await supabase.from('profiles').select('role,full_name,team').eq('id', user.id).maybeSingle()
  ...
  if (data.kind === 'ceo') {
    ...
    items = data.partners.filter((item) => normalize(item.name) === normalize(profile?.full_name))
