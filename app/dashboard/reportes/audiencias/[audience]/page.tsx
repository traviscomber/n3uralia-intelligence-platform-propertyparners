...
export default async function AudiencePage({ params, searchParams }: { params: Promise<{ audience: string }>; searchParams: Promise<{ branch?: string }> }) {
  ...
  return <div className="pp-print-report mx-auto max-w-7xl space-y-6 pb-16"><header className="border-b border-[var(--n3-line)] pb-6"><div className="print-hidden flex flex-wrap items-center justify-between gap-3"><Link href={backHref} className="text-xs font-semibold text-[var(--n3-teal)]">← {backLabel}</Link><PrintReportButton /></div><p className="print-only text-xs font-semibold uppercase tracking-[0.18em]">Property Partners Vitacura</p><h1 className="mt-4 text-3xl font-semibold text-[var(--n3-text-light)] sm:text-4xl">{title}</h1><div className="mt-3 grid gap-1 text-sm text-[var(--n3-text-muted)] sm:grid-cols-2"><p>Período: enero–junio 2026</p><p className="sm:text-right">Corte comercial: junio 2026</p><p>Operación: venta · Vitacura</p><p className="sm:text-right">Fuente: presentaciones auditadas 2026</p></div></header>
