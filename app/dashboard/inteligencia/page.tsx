...
function Evidence({ children }: { children: React.ReactNode }) {
  return <p className="mt-3 border-l-2 border-[#d7332b] pl-3 text-xs leading-5 text-[var(--n3-text-muted)]">{children}</p>
}
    ...
    <div className="mx-auto max-w-[1600px] space-y-7 pb-16 text-[var(--n3-text-light)]">
      <header className="overflow-hidden border border-white/10 bg-black text-white">
        <div className="grid gap-8 p-7 lg:grid-cols-[minmax(0,1fr)_360px] lg:p-10">
          <div>
            <p className="text-[11px] font-bold uppercase tracking-[0.24em] text-[var(--n3-teal)]">Gestión corporativa · uso interno</p>
            <h1 className="mt-3 text-3xl font-semibold tracking-tight lg:text-4xl">Resultados y control de gestión</h1>
            <p className="mt-3 max-w-3xl text-sm leading-6 text-[var(--n3-text-muted)]">Resumen para CEO y dirección. Corte acumulado enero–junio 2026, con comparación 2025, cartera, actividad comercial y alertas de calidad.</p>
          </div>
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
        {cards.map(([label, value, detail]) => (
          <article key={label} className="border border-[var(--n3-line)] bg-[var(--n3-deep)] p-4">
            <p className="text-[10px] font-bold uppercase tracking-[0.15em] text-[var(--n3-text-muted)]">{label}</p>
            <p className="mt-2 text-2xl font-semibold text-[var(--n3-text-light)]">{value}</p>
            <p className="mt-2 text-[11px] leading-4 text-[var(--n3-text-muted)]">{detail}</p>
          </article>
        ))}
      </section>

      <section className="border border-[var(--n3-line)] bg-[var(--n3-deep)]">
        <div className="flex flex-wrap items-end justify-between gap-3 border-b border-[var(--n3-line)] p-5"><div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#d7332b]">Revisión de gestión</p><h2 className="mt-2 text-xl font-semibold">Excepciones del corte</h2></div><p className="text-xs text-[var(--n3-text-muted)]">Valores reportados; no incluyen diagnóstico causal</p></div>
        <div className="grid gap-px bg-[var(--n3-line)] md:grid-cols-3">
          <article className="bg-[var(--n3-deep)] p-5"><p className="text-[10px] font-bold uppercase tracking-[0.15em] text-[var(--n3-text-muted)]">Cartera vs. meta junio</p><p className="mt-2 text-2xl font-semibold">{percent(stockAttainment)}</p><p className="mt-2 text-xs leading-5 text-[var(--n3-text-muted)]">{number(june.stockCount)} propiedades informadas frente a meta de {number(juneStockTarget)}.</p><Link href="/dashboard/control" className="mt-4 inline-flex text-xs font-semibold text-[#d7332b]">Revisar control de gestión →</Link></article>
          <article className="bg-[var(--n3-deep)] p-5"><p className="text-[10px] font-bold uppercase tracking-[0.15em] text-[var(--n3-text-muted)]">Leads sin gestión +90 días</p><p className="mt-2 text-2xl font-semibold">{number(crm.latestLeadSnapshot.staleOver90)}</p><p className="mt-2 text-xs leading-5 text-[var(--n3-text-muted)]">Parte de los {number(crm.latestLeadSnapshot.staleOver15Total)} leads con más de 15 días sin gestión.</p><Link href="/dashboard/datos-crm" className="mt-4 inline-flex text-xs font-semibold text-[#d7332b]">Revisar CRM →</Link></article>
          <article className="bg-[var(--n3-deep)] p-5"><p className="text-[10px] font-bold uppercase tracking-[0.15em] text-[var(--n3-text-muted)]">Calidad de metas</p><p className="mt-2 text-2xl font-semibold">{targets.quality.criticalCount} críticas</p><p className="mt-2 text-xs leading-5 text-[var(--n3-text-muted)]">{targets.quality.issueCount} observaciones conservadas en Metas 2026.</p><Link href="/dashboard/metas" className="mt-4 inline-flex text-xs font-semibold text-[#d7332b]">Revisar metas →</Link></article>
        </div>
      </section>
      ...
      <section className="grid gap-4 xl:grid-cols-3">
        <article className="border border-[var(--n3-line)] bg-[var(--n3-deep)] p-5">
          ...
          <p className="mt-3 text-sm leading-6 text-[var(--n3-text-muted)]">El primer semestre registra {crm.ytd.salesCount} ventas y {uf(crm.ytd.salesUf)}. Frente al mismo período de 2025, los cierres aumentan {percent(crm.ytd.comparison2025.salesChangePct)} y el volumen {percent(crm.ytd.comparison2025.salesUfChangePct)}.</p>
        ...
        <article className="border border-[var(--n3-line)] bg-[var(--n3-deep)] p-5">
          ...
          <p className="mt-3 text-sm leading-6 text-[var(--n3-text-muted)]">El cociente UF/cierres pasa de {uf(ticket2025)} en el primer semestre de 2025 a {uf(ticket2026)} en 2026, una diferencia calculada de {percent(ticketChange)}.</p>
        ...
        <article className="border border-[var(--n3-line)] bg-[var(--n3-deep)] p-5">
          ...
          <p className="mt-3 text-sm leading-6 text-[var(--n3-text-muted)]">El stock comparable baja de {number(january.stockCount)} en enero a {number(june.stockCount)} en junio: {number(Math.abs(crm.ytd.stockChange))} propiedades, equivalentes a {percent(stockChangePct)}.</p>
          <Evidence>Fuente: snapshots mensuales CRM. La relación no determina la causa de la reducción.</Evidence>
        </article>
      </section>

      <section className="border border-[var(--n3-line)] bg-[var(--n3-deep)]">
        <div className="flex flex-wrap items-end justify-between gap-3 border-b border-[var(--n3-line)] bg-black p-5 text-white">
          <div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[var(--n3-teal)]">Seguimiento mensual</p><h2 className="mt-2 text-xl font-semibold">Actividad comercial</h2></div>
          <p className="text-xs text-[var(--n3-text-muted)]">Los cocientes mensuales no son conversiones de cohorte</p>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full min-w-[980px] text-left text-xs">
            <thead className="bg-[var(--n3-deep)] text-[var(--n3-text-muted)]"><tr><th className="px-4 py-3">Período</th><th>Cierres</th><th>UF</th><th>Ticket medio</th><th>Captaciones</th><th>Leads</th><th>Requerimientos</th><th>Visitas</th><th>Realizadas</th><th>Stock</th><th>Cobertura</th></tr></thead>
        ...
        <article className="border border-[var(--n3-line)] bg-[var(--n3-deep)] p-5">
          ...
          <div className="mt-5 grid grid-cols-2 gap-px bg-[var(--n3-line)]">
            {[['Mayo', may], ['Junio', june]].map(([label, raw]) => {
              ...
              return <div key={label as string} className="bg-[var(--n3-deep)] p-4"><p className="font-semibold">{label as string}</p><p className="mt-3 text-2xl font-semibold">{month.salesCount} cierres</p><p className="text-xs text-[var(--n3-text-muted)]">{number(month.newLeadsCount)} leads · {number(month.requirementsCount)} requerimientos</p></div>
            })}
          </div>
      ...
      <section className="grid gap-px bg-[var(--n3-line)] lg:grid-cols-3">
        <article className="bg-[var(--n3-deep)] p-5"><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[var(--n3-text-muted)]">Mercado</p><p className="mt-2 text-3xl font-semibold">{number(portalListings)}</p><p className="mt-1 text-sm text-[var(--n3-text-muted)]">filas de publicaciones auditadas</p><p className="mt-4 text-xs leading-5 text-[var(--n3-text-muted)]">{number(valuation.sourceReconciliation.currentPortalSaleEligibleListings)} publicaciones sin señal explícita de arriendo y {number(valuation.sourceReconciliation.portalListingsQuarantinedByRentIndicator)} señales de arriendo excluidas.</p></article>
        <article className="bg-[var(--n3-deep)] p-5"><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[var(--n3-text-muted)]">Registro</p><p className="mt-2 text-3xl font-semibold">{number(market.cross.cbrs.rows)}</p><p className="mt-1 text-sm text-[var(--n3-text-muted)]">inscripciones CBRS Vitacura</p><p className="mt-4 text-xs leading-5 text-[var(--n3-text-muted)]">La clave evento + rol es única en los {number(market.cross.cbrs.candidateKeyCardinality.event_plus_rol.unique)} registros.</p></article>
        <article className="bg-[var(--n3-deep)] p-5"><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[var(--n3-text-muted)]">Territorio</p><p className="mt-2 text-3xl font-semibold">{number(market.kml.geometryAudit.polygonCount)}</p><p className="mt-1 text-sm text-[var(--n3-text-muted)]">polígonos de barrios</p><p className="mt-4 text-xs leading-5 text-[var(--n3-text-muted)]">{number(market.cross.cbrs.barrioComparison.coordinate_agrees_with_barrio)} registros coinciden entre coordenada y barrio asignado.</p></article>
      </section>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <article className="border border-[var(--n3-line)] bg-[var(--n3-deep)] p-5">
          ...
          <div className="mt-5 space-y-3 text-sm leading-6 text-[var(--n3-text-muted)]"><p><strong>Departamento:</strong> superficie útil + terraza al 50%; valor comercial calculado sobre superficie útil y UF/m² aplicada.</p><p><strong>Casa:</strong> valor de construcción + terreno; superficie comparable con terreno ponderado al 25%.</p></div>
      ...
      <section className="border border-[var(--n3-line)] bg-black p-6 text-white">
        <div className="flex flex-wrap items-start justify-between gap-5"><div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[var(--n3-teal)]">Detalle y respaldo</p><h2 className="mt-2 text-xl font-semibold">Fuentes del reporte</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--n3-text-muted)]">Los indicadores se calculan desde las capas auditadas. El detalle de CRM, metas, presentaciones y mercado está disponible en los módulos correspondientes.</p></div><div className="flex flex-wrap gap-2">{[
