...
function blankComparable(index: number, type: ValuationSubject['propertyType']): ValuationComparable {
  return {
    id: `cmp-${Date.now()}-${index}`, sourceType: 'Portal', sourceReference: '', address: '', neighborhood: '',
...
export default function ValuationPage() {
  ...
  function addComparable(sourceType: ValuationComparable['sourceType'] = 'Portal') {
  ...
  return <IntelligencePage>
    ...
    <MethodologyNote>Casas: valor comercial por m² construidos + terreno y UF/m² definidos por el valorizador; comparables ponderados con terreno/4. Departamentos: valor comercial por m² útiles × UF/m² útil; oferta pondera 50% de terraza y CBRS utiliza m² útiles. Los factores cualitativos quedan para revisión humana y no alteran automáticamente el precio.</MethodologyNote>
    ...
    <section><SectionHeading eyebrow="01 · Ficha" title="Propiedad a valorizar" /><IntelligencePanel eyebrow="Sujeto" title="Antecedentes básicos" description="Equivalente digital de la sección inicial de las plantillas canónicas."><div className="grid gap-4 p-5 md:grid-cols-3">
      ...
      {subject.propertyType === 'Departamento' ? <>
        <NumberField label="M² útiles" value={subject.usefulAreaM2} onChange={(value) => updateSubject('usefulAreaM2', value)} suffix="m²" step={0.1} min={0} />
        <NumberField label="M² terraza" value={subject.terraceAreaM2} onChange={(value) => updateSubject('terraceAreaM2', value)} suffix="m²" step={0.1} min={0} />
        <NumberField label="UF/M² útil valorización" value={subject.usefulRateUfM2} onChange={(value) => updateSubject('usefulRateUfM2', value)} suffix="UF/m²" step={0.1} min={0} />
        ...
        <NumberField label="UF/M² construido" value={subject.builtRateUfM2} onChange={(value) => updateSubject('builtRateUfM2', value)} suffix="UF/m²" step={0.1} min={0} />
    ...
    <section><div className="flex flex-wrap items-end justify-between gap-4"><SectionHeading eyebrow="02 · Comparables" title="Oferta y ventas reales" /><div className="mb-5 flex gap-2"><button type="button" onClick={() => addComparable('Portal')} className="flex items-center gap-2 border border-[var(--n3-line)] px-3 py-2 text-xs font-semibold hover:border-[#d7332b]"><Plus size={14} />Oferta</button><button type="button" onClick={() => addComparable('CBRS')} className="flex items-center gap-2 border border-[var(--n3-line)] px-3 py-2 text-xs font-semibold hover:border-[#d7332b]"><Plus size={14} />Venta CBRS</button></div></div>
      {!comparables.length ? <div className="border border-dashed border-[var(--n3-line)] p-8 text-center text-sm text-[var(--n3-text-muted)]">Agrega comparables verificables de Portal/TocToc y ventas CBRS. El UF/m² se calcula automáticamente.</div> : null}
      <div className="space-y-4">{comparables.map((item, index) => {
        const canonicalUfM2 = calculateCanonicalComparableUfM2(item)
        return <IntelligencePanel key={item.id} eyebrow={`${item.sourceType === 'CBRS' ? 'Venta' : 'Oferta'} ${index + 1}`} title={item.address || 'Sin dirección'} description={`UF/m² canónico: ${canonicalUfM2 > 0 ? canonicalUfM2.toLocaleString('es-CL') : 'pendiente'}`}><div className="grid gap-3 p-4 md:grid-cols-3 xl:grid-cols-5">
          ...
          <DateField label="Fecha venta" value={item.transactionDate} onChange={(value) => updateComparable(index, { transactionDate: value })} />
          ...
          <NumberField label={item.sourceType === 'CBRS' ? 'Precio venta' : 'Precio publicado'} value={item.priceUf} onChange={(value) => updateComparable(index, { priceUf: value ?? 0 })} suffix="UF" min={0} />
          ...
          <NumberField label="Distancia" value={item.distanceMeters} onChange={(value) => updateComparable(index, { distanceMeters: value })} suffix="m" min={0} />
          <div className="border border-[var(--n3-line)] bg-[#080d0d] px-3 py-3"><FieldLabel>UF/M² calculado</FieldLabel><strong className="text-sm">{canonicalUfM2 > 0 ? canonicalUfM2.toLocaleString('es-CL') : '—'}</strong></div>
    ...
    <section><SectionHeading eyebrow="03 · Resultado" title="Valor comercial y contraste" /><MetricGrid>
      ...
      <MetricCard label="CBRS promedio" value={result?.cbrsSummary.averagePriceUf ? `${Math.round(result.cbrsSummary.averagePriceUf).toLocaleString('es-CL')} UF` : 'Sin muestra'} detail={result ? `Variación valor: ${pct(result.salePriceVarianceVsCbrsAveragePct)}` : 'Ventas registradas.'} />
