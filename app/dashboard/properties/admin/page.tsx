...
function formatDate(value: string | null) {
  return value ? formatPropertyPartnersDateTime(value) : 'Sin fecha'
}

function isExplicitVitacuraAddress(value: string | null | undefined) {
  return String(value || '').toLocaleLowerCase('es-CL').includes('vitacura')
}
...
async function createAssignment(formData: FormData) {
  ...
  const { data: property, error: propertyError } = await supabase.from('market_properties').select('id,property_type,normalized_address').eq('id', propertyId).maybeSingle()
  ...
  if (!isExplicitVitacuraAddress(property.normalized_address)) throw new Error('La versión 1 sólo permite asignar casas con evidencia territorial explícita de Vitacura.')
...
export default async function PropertyAssignmentAdminPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  let propertiesQuery = supabase
    ...
    .select('id,normalized_address,property_type,bedrooms,bathrooms,useful_area_m2,identity_status,last_seen_at')
    .eq('property_type', 'Casa')
    .ilike('normalized_address', '%Vitacura%')
