...
export async function GET(request: Request) {
  ...
  if (!process.env.CRON_SECRET || authorization !== `Bearer ${process.env.CRON_SECRET}`) return NextResponse.json({ error: 'No autorizado' }, { status: 401 })
  ...
  for (const schedule of schedules ?? []) {
    ...
    const snapshot = { period: period.start.slice(0,7), generatedBy: 'scheduled-automation', definitions, entities: scopedEntities.map(entity => ({ ...entity, metrics: (metrics ?? []).filter(metric => metric.entity_id === entity.id) })), alerts: (alerts ?? []).filter(alert => !schedule.entity_id || alert.entity_id === schedule.entity_id) }
    ...
    const recipients: string[] = Array.isArray(schedule.recipients) ? schedule.recipients.map((value: unknown) => String(value)).filter(Boolean) : []
    if (recipients.length) await supabase.from('management_report_distributions').insert(recipients.map((recipient: string) => ({ report_run_id: report.id, recipient, channel:'email', status:'pending' })))
