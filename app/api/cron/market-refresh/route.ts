...
async function reconcileRemovedListings(
  ...
  for (let offset = 0; ; offset += 1000) {
    const { data, error } = await supabase
      ...
      .select('source_id,source_listing_id,property_id,operation,url,title,raw_address,normalized_address,latitude,longitude,price_clp,price_uf,price_uf_m2,published_at,raw_payload,status')
