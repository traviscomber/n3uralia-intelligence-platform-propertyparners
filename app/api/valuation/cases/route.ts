...
function validateComparable(item: ValuationComparable, index: number) {
  ...
  if (!Number.isFinite(item.adjustmentPct) || item.adjustmentPct < -35 || item.adjustmentPct > 35) return `${label}: el ajuste documentado debe estar entre -35% y 35%.`
  ...
  if (item.selected && item.priceUf > 0) {
    ...
    if (calculateCanonicalComparableUfM2(item) <= 0) return `${label}: faltan superficies para calcular el UF/m² canónico.`
  }
  return null
}
