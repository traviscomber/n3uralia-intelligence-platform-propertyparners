...
type Candidate = {
  source_type:string; source_reference:string; comparable_property_id:string|null; source_transaction_id:string|null;
  source_listing_id:string|null; transaction_date:string|null; observed_at:string|null; address:string|null;
  neighborhood:string|null; property_type:string|null; useful_area_m2:number|null; built_area_m2:number|null;
  land_area_m2:number|null; bedrooms:number|null; bathrooms:number|null; parking_spaces:number|null;
  price_uf:number|null; price_uf_m2:number|null; distance_meters:number|null; similarity_score:number; evidence:unknown[]
}
...
async function recalculateCase(supabase:Awaited<ReturnType<typeof createClient>>,caseId:string){
  const { data:selected,error } = await supabase.from('valuation_comparables')
    .select('adjusted_value_uf,price_uf,selected,match_status').eq('valuation_case_id',caseId)
    .eq('selected',true).eq('match_status','accepted')
  if(error) throw error
  const values=(selected || []).map(row=>Number(row.adjusted_value_uf ?? row.price_uf)).filter(value=>Number.isFinite(value)&&value>0)
  if(!values.length){
    const { error:clearError } = await supabase.from('valuation_cases').update({
      base_value_uf:null,estimated_value_uf:null,low_value_uf:null,high_value_uf:null,confidence:'low',
      warnings:['No existen comparables aceptados con valores utilizables.'],updated_at:new Date().toISOString(),
    }).eq('id',caseId)
  ...
  const { error:updateError } = await supabase.from('valuation_cases').update({
    base_value_uf:estimate,estimated_value_uf:estimate,low_value_uf:low,high_value_uf:high,confidence,
    methodology_version:'market_comparables_v1',warnings:values.length<3?['Se requieren al menos tres comparables aceptados para solicitar revisión.']:[],
...
export async function GET(_request:Request,context:{ params:Promise<{id:string}> }){
  ...
  return NextResponse.json({
    valuationCase,comparables:comparables || [],decisions:decisions || [],
    permissions:{ canEditComparables:ELEVATED_ROLES.includes(access.role)||valuationCase.status==='draft',canApprove:ELEVATED_ROLES.includes(access.role),canIssue:ELEVATED_ROLES.includes(access.role) },
  })
}
...
export async function POST(request:Request,context:{ params:Promise<{id:string}> }){
  ...
  if(caseError) return NextResponse.json({ error:caseError.message },{ status:500 })
  if(!valuationCase) return NextResponse.json({ error:'Valorización no encontrada' },{ status:404 })
  const elevated=ELEVATED_ROLES.includes(access.role)
  ...
  if(action==='generate'){
    ...
    const rows=candidateRows.filter(candidate=>{ const key=candidateKey(candidate); if(seenKeys.has(key)) return false; seenKeys.add(key); return true })
    let rank=Number(existingRows?.[0]?.rank || 0)
    const inserts=rows.map(candidate=>({
      valuation_case_id:id,comparable_property_id:candidate.comparable_property_id,rank:++rank,similarity_score:candidate.similarity_score,
      distance_meters:candidate.distance_meters,base_value_uf:candidate.price_uf,adjusted_value_uf:candidate.price_uf,adjustments:[],
      evidence:candidate.evidence || [],contradictions:[],match_status:'candidate',source_type:candidate.source_type,
      source_reference:candidate.source_reference,transaction_date:candidate.transaction_date,address:candidate.address,neighborhood:candidate.neighborhood,
      property_type:candidate.property_type,useful_area_m2:candidate.useful_area_m2,built_area_m2:candidate.built_area_m2,
      land_area_m2:candidate.land_area_m2,bedrooms:candidate.bedrooms,bathrooms:candidate.bathrooms,parking_spaces:candidate.parking_spaces,
      price_uf:candidate.price_uf,price_uf_m2:candidate.price_uf_m2,selected:false,adjustment_pct:0,
      source_transaction_id:candidate.source_transaction_id,source_listing_id:candidate.source_listing_id,
      source_observed_at:candidate.observed_at,source_methodology_version:'valuation_candidate_pool_v1',
    }))
    if(inserts.length){ const { error:insertError }=await access.supabase.from('valuation_comparables').insert(inserts); if(insertError) return NextResponse.json({ error:insertError.message },{ status:500 }) }
    const duplicatesSkipped=candidateRows.length-inserts.length
    const { error:logError } = await access.supabase.from('valuation_decision_log').insert({ valuation_case_id:id,action:'candidate_generated',actor_id:access.user.id,new_state:{ candidateCount:inserts.length,duplicatesSkipped,methodology:'valuation_candidate_pool_v1' },reason:String(body?.reason || 'Generación automática desde evidencia del Módulo I') })
  ...
  if(action==='select'){
    ...
    const patch={ selected:true,match_status:'accepted',exclusion_reason:null,adjustment_pct:adjustmentPct,adjustment_notes:String(body?.notes || '')||null,adjusted_value_uf:adjusted,selected_by:access.user.id,selected_at:new Date().toISOString(),excluded_by:null,excluded_at:null }
    const { error }=await access.supabase.from('valuation_comparables').update(patch).eq('id',comparableId).eq('valuation_case_id',id)
    if(error) return NextResponse.json({ error:error.message },{ status:500 })
    const { error:logError }=await access.supabase.from('valuation_decision_log').insert({ valuation_case_id:id,comparable_id:comparableId,action:'comparable_selected',actor_id:access.user.id,previous_state:previous,new_state:patch,reason:String(body?.reason || body?.notes || '')||null })
    if(logError) return NextResponse.json({ error:logError.message },{ status:500 })
    return NextResponse.json({ updated:true,valuation:await recalculateCase(access.supabase,id) })
  }
  ...
  if(action==='exclude'){
    ...
    const { error }=await access.supabase.from('valuation_comparables').update(patch).eq('id',comparableId).eq('valuation_case_id',id)
    if(error) return NextResponse.json({ error:error.message },{ status:500 })
    const { error:logError }=await access.supabase.from('valuation_decision_log').insert({ valuation_case_id:id,comparable_id:comparableId,action:'comparable_excluded',actor_id:access.user.id,previous_state:previous,new_state:patch,reason })
    if(logError) return NextResponse.json({ error:logError.message },{ status:500 })
    return NextResponse.json({ updated:true,valuation:await recalculateCase(access.supabase,id) })
  }
