...
export async function POST(request: Request) {
  ...
  if (type === 'metric') {
    ...
    const record = {
      ...
      period_end: body.periodEnd,
      value,
      source_name: String(body.sourceName || 'Carga administrativa'),
      source_reference: body.sourceReference || null,
      source_cutoff_at: body.sourceCutoffAt || new Date().toISOString(),
      quality_status: body.qualityStatus || 'verified',
      evidence: body.evidence || {},
    }
    if (!record.entity_id || !record.metric_code || !record.period_start || !record.period_end || !Number.isFinite(value)) return NextResponse.json({ error: 'Métrica incompleta' }, { status: 400 })
